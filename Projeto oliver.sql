--Dependentes
select id, nome, cpf from cliente where id in( SELECT id_conta FROM cliente_conta where dependente like '1%');
--5 contas menos transaçoes
SELECT conta.numero, COUNT (transacao.id_cliente_conta) as tc
from transacao
	inner join cliente_conta on cliente_conta.id = transacao.id_cliente_conta
    INNER join conta on conta.id = cliente_conta.id_conta
GROUP by conta.numero
order by tc ASC
limit 5 
--5 contas mais transaçoes
...
--Saldo
select e.numero 'Numero da Conta',depositos.total_depositos 'Entradas', debitos.total_debitos 'Saques',
(depositos.total_depositos-debitos.total_debitos) as Total 
from conta AS e
join (select t.numero AS b,SUM(valor) as total_depositos 
from transacao 
join cliente_conta on cliente_conta.id=transacao.id_cliente_conta 
join conta AS t on t.id=cliente_conta.id_conta 
where transacao.id_tipo_transacao=1 
group by t.numero) as depositos 
on depositos.b=e.numero 
join (select f.numero as l,SUM(valor) as total_debitos 
from transacao 
join cliente_conta on cliente_conta.id=transacao.id_cliente_conta 
join conta AS f on f.id=cliente_conta.id_conta 
where transacao.id_tipo_transacao<>1 
group by f.numero)as debitos 
on debitos.l=e.numero;
